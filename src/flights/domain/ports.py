...
class SeatLockStore(Protocol):
    ...
    # ``get`` ignores TTL so BookingService.commit can distinguish
    # "expired" (stored but past ``expires_at``) from "unknown" (never
    # issued). Mirrors ``QuoteStore.get`` — both driven ports expose a
    # TTL-ignoring lookup alongside their TTL-enforcing validator so the
    # service can return the appropriate HTTP status (410 vs 404).
    def get(self, lock_id: str) -> object | None: ...
